...
6. `pnpm admin:setup` çalıştırın. Şema ve mevcut ürünler ilk kurulumda aktarılır. Fiyat ve stoklar bilinmediği için boş bırakılır. Sonraki çalıştırmalar düzenlenen/silinen ürünleri geri yüklemez. Komut üretim build sırasında otomatik çalışmaz.
7. Görseller için aşağıdaki sürücülerden birini seçin. Ardından `pnpm dev` ile açın ve `/admin/giris` sayfasına gidin.
...
- Ödeme hâlâ kapalıdır; bu değişiklik ödeme/talep akışını açmaz, canlı fiyatları değiştirmez ve otomatik deployment yapmaz.
